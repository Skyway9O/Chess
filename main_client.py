...
couleur_joueur1 = "blanc" if couleur_joueur2 == "noir" else "noir"
tourJ = 0
...
def selection_case(x : int, y : int):
    ...
    if piece_selection == True and cases[case_selection][0] == (roi_blanc if couleur_joueur2 == "blanc" else roi_noir) and cases[case_clique][0] == (tour_blanc if couleur_joueur2 == "blanc" else tour_noir) and deja_roque == False: #si la pièce sélectionnée est un roi et que le joueur clique sur une tour (roque)
        if ((cases[case_clique][2] == 63) if couleur_joueur2 == "blanc" else cases[case_clique][2] == 56) and p_roque == False:  #si le joueur clique sur la tour de droite
            if ((cases[61][0] == "vide" and cases[62][0] == "vide") if couleur_joueur2 == "blanc" else (cases[57][0] == "vide" and cases[58][0] == "vide")):
                petit_roque(couleur_joueur2)
                case_selection = None #on désélectionne la case sélectionnée
                client.envoie("petit_roque")
        elif ((cases[case_clique][2] == 56) if couleur_joueur2 == "blanc" else cases[case_clique][2] == 63) and g_roque == False: #si le joueur clique sur la tour de gauche
            if ((cases[57][0] == "vide" and cases[58][0] == "vide" and cases[59][0] == "vide") if couleur_joueur2 == "blanc" else (cases[60][0] == "vide" and cases[61][0] == "vide" and cases[62][0] == "vide")):
                grand_roque(couleur_joueur2)
                case_selection = None #on désélectionne la case sélectionnée
                client.envoie("grand_roque")

    elif cases[case_clique][1] == "blanc" and tour_de_jeu == "blanc" and couleur_joueur2 == "blanc": #si la piece est blanche et que c'est le tour des blancs
    ...
    elif cases[case_clique][1] == "noir" and tour_de_jeu == "noir" and couleur_joueur2 == "noir": #si la piece est noire et que c'est le tour des noirs
        piece_selection = True #la sélection d'une pièce est vraie
        cases_possibles = [] #on vide la liste des cases possibles pour ne pas avoir de problèmes avec les cases possibles d'autres pièces
        case_selection = case_clique #permet de garder en mémoire la case de la pièce sélectionnée 
        deplacement_possible(case_clique)
            ...
            if echec == True:
                client.envoie(str(case_selection)+","+str(case_clique)+","+"echec")
                case_selection = None #on remet la case sélectionnée à "aucune"
            elif echec == False:
                if cases[case_clique][0] == (pion_blanc if couleur_joueur2 == "blanc" else pion_noir) and case_clique // 8 == 0:
                    ...
                    if cases[case_clique][0] == roi_noir or cases[case_clique][0] == roi_blanc:
                        p_roque = True
                        g_roque = True
                    client.envoie(str(case_selection)+","+str(case_clique))
                    case_selection = None #on remet la case sélectionnée à "aucune"
...
def deplacement_possible(case_select : int):
    ...
    if piece == (pion_blanc if couleur_joueur2 == "blanc" else pion_noir): #si c'est un pion
        pion(case_select, "joueur") #on appelle la fonction pion

    elif piece == (tour_blanc if couleur_joueur2 == "blanc" else tour_noir): #si c'est une tour
        #Deplacement haut
    ...
    elif piece == (cavalier_blanc if couleur_joueur2 == "blanc" else cavalier_noir): #si c'est un cavalier
    ...
    elif piece == (fou_blanc if couleur_joueur2 == "blanc" else fou_noir): #si c'est un fou
        # Direction N-E
    ...
    elif piece == (roi_blanc if couleur_joueur2 == "blanc" else roi_noir): #si c'est un roi
    ...
    elif piece == (reine_blanc if couleur_joueur2 == "blanc" else reine_noir): #si c'est une reine
...
def pion(case_select : int, type_joueur : str):
    ...
    if type_joueur == "joueur":
        ...
        if cases[case_select-7][0] != "vide" and cases[case_select-7][1] == couleur_joueur1: #si la case de déplacement du pion est occupée par une pièce adverse
        ...
        if cases[case_select-9][0] != "vide" and cases[case_select-9][1] == couleur_joueur1: #si la case de déplacement du pion est occupée par une pièce adverse
        ...
        if cases[case_select+7][2] <= 63:
            cases_precalculees.append(cases[case_select+7]) #on ajoute la case à la liste des cases d'echec
        if cases[case_select+9][2] <= 63: 
            cases_precalculees.append(cases[case_select+9]) #on ajoute la case à la liste des cases d'echec
...
def cavalier(case_select : int, index_diff_case_visee : int ,difference_colonne : int,  type_joueur : str):
    ...
    if (63 >= (case_select+index_diff_case_visee) >= 0) and ((case_select+index_diff_case_visee) % 8) == (case_select % 8) + difference_colonne: #si la case pour manger un adversaire est bien égale à la différence de colonne
        if cases[case_select+index_diff_case_visee][1] != (("blanc" if couleur_joueur2 == "blanc" else "noir") if type_joueur == "joueur" else ("noir" if couleur_joueur2 == "blanc" else "blanc") ): #si la case de déplacement du cavalier est occupée par une pièce adverse
...
def roi(case_select : int, index_diff_case_visee : int, difference_colonne : int, type_joueur : str):
    ...
    if 0 < (case_select+index_diff_case_visee) <= 63 and ((case_select+index_diff_case_visee) % 8) == (case_select % 8) + difference_colonne: #si la case de destination est bien dans le plateau
        if cases[case_select+index_diff_case_visee] not in cases_precalculees and type_joueur == "joueur": #si la case de destination n'est pas dans la liste des cases echec
            if cases[case_select+index_diff_case_visee][1] != couleur_joueur2: #si la case de destination n'est pas occupée par une pièce de la même couleur que le roi
                cases_possibles.append(cases[case_select+index_diff_case_visee])
        elif type_joueur == "auto": #si c'est l'ordinateur qui calcule les coups possibles afin d'éviter de jouer des pièces alors qu'il y a échec
            cases_precalculees.append(cases[case_select+index_diff_case_visee]) #on ajoute la case à la liste des cases echec
...
def deplacement_direction(idx_case,x,y, type_joueur):
    ...
    while (case_possible):
        ...
        if cases[index_deplacement][0] == "vide":
            ...
            case_possible = (index_deplacement >=0 and index_deplacement <=63 and (True if x == 0 else (index_deplacement+modulo_offset)%8 !=0))
        elif cases[index_deplacement][1] != couleur_joueur2:
            if type_joueur == "joueur":
                cases_possibles.append(cases[index_deplacement])
            elif type_joueur == "auto":
                cases_precalculees.append(cases[index_deplacement])
...
def grand_roque(couleur_joueur : str):
    ...
    if couleur_joueur == "blanc":
        ...
        cases[56][0:2], cases[59][0:2] = cases[59][0:2], cases[56][0:2]
        new_case = 58
    elif couleur_joueur == "noir":
...
def lance():
    """Cette fonction permet de lancer la connexion au serveur"""
    global couleur_joueur2, couleur_joueur1
    client.connect()
    couleur_joueur2 = client.couleur_joueur2
    couleur_joueur1 = "blanc" if couleur_joueur2 == "noir" else "noir"
    if client.couleur_joueur2 == "noir":
        cases.reverse()
...
def test_echec():
    ...
    for i in cases: #on parcourt toutes les cases
        if i[0] == (pion_blanc if couleur_joueur2 == "noir" else pion_noir): #si la pièce est un pion
            pion(i[2], "auto")
        elif i[0] == (tour_blanc if couleur_joueur2 == "noir" else tour_noir): #si la pièce est une tour
            #Deplacement haut
            ...
            #Deplacement gauche
            deplacement_direction(i[2], -1, 0, "auto") #on appelle la fonction deplacement_direction
        elif i[0] == (cavalier_blanc if couleur_joueur2 == "noir" else cavalier_noir): #si la pièce est un cavalier
            ...
            cavalier(i[2], 15, -1, "auto") #on appelle la fonction cavalier
            cavalier(i[2], 17, 1, "auto") #on appelle la fonction cavalier
        elif i[0] == (fou_blanc if couleur_joueur2 == "noir" else fou_noir): #si la pièce est un fou
            # Direction N-E
            ...
            # Direction S-O
            deplacement_direction(i[2], -1, 1, "auto") #on appelle la fonction deplacement_direction
        elif i[0] == (roi_blanc if couleur_joueur2 == "noir" else roi_noir): #si la pièce est un roi
            ...
            roi(i[2], 8, 0, "auto") #on appelle la fonction roi
            roi(i[2], 9, 1, "auto") #on appelle la fonction roi
        elif i[0] == (reine_blanc if couleur_joueur2 == "noir" else reine_noir): #si la pièce est une reine
            ##deplacement comme un fou##
            # Direction N-E
            deplacement_direction(i[2], 1, -1, "auto") #on appelle la fonction deplacement_direction
            # Direction N-O
...
def fnc_echec():
    ...
    global echec 
    for i in cases: #on parcourt toutes les cases
        if i[0] == (roi_blanc if couleur_joueur2 == "blanc" else roi_noir): #si la pièce est un roi

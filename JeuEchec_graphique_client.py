...
def dessine_tout():
    ...
    if main_client.tourJ != 0:
        coup_joue = dessine_coup_joue()

    bleu = dessine_coups_possibles()

    if main_client.case_selection != None:
        gris = dessine_case_selectionnee()
    if main_client.echec == True:
        img_piece = dessine_echec()
        label_info.configure(text="⚠ Échec ⚠")
    elif main_client.echec == False:
        label_info.configure(text="")
    if main_client.utilisateur_a_quitte == True:
        adversaire_a_quitter()

    img_piece = dessine_element()

    if main_client.case_piece_demandee != None:
        pop_up_choix_piece(main_client.case_piece_demandee)
...
def dessine_element():
    ...
    for y in range(constant.NB_Row):
        for x in range(constant.NB_Col):
            num_case_piece = main_client.cases[y*8+x][0]
...
def dessine_coups_possibles():
    ...
    for i in main_client.cases_possibles:
        x = i[2] % 8
        y = i[2] // 8
        carre_possible = canvas.create_rectangle(x*constant.CELL_Size, y*constant.CELL_Size, x*constant.CELL_Size+100, y*constant.CELL_Size+100, fill="#009eff")


def dessine_case_selectionnee():
    """Cette fonction permet de dessiner un carré de couleur qui montre la case selectionnée"""
    carre_selectionnee = canvas.create_rectangle(main_client.case_selection%8*constant.CELL_Size, main_client.case_selection//8*constant.CELL_Size, main_client.case_selection%8*constant.CELL_Size+constant.CELL_Size, main_client.case_selection//8*constant.CELL_Size+constant.CELL_Size, fill="#cfcfcf")


def dessine_coup_joue():
    """dessine une case jaune pâle sur la dernière case jouée"""
    carre_coup_joue = canvas.create_rectangle(main_client.new_case%8*constant.CELL_Size, main_client.new_case//8*constant.CELL_Size, main_client.new_case%8*constant.CELL_Size+constant.CELL_Size, main_client.new_case//8*constant.CELL_Size+constant.CELL_Size, fill="#ffeb6d")


def dessine_echec():
    """dessine une case rouge sous le roi quand celui-ci est en échec"""
    for i in main_client.cases:
        if i[0] == (main_client.roi_blanc if main_client.couleur_joueur2 == "blanc" else main_client.roi_noir):
            x = i[2] % 8
            y = i[2] // 8
            echec = canvas.create_rectangle(x*constant.CELL_Size, y*constant.CELL_Size, x*constant.CELL_Size+100, y*constant.CELL_Size+100, fill="#f85959")


def dessine_tour():
    """Cette fonction permet de dessiner un carré de couleur qui montre le tour de jeu"""
    if main_client.tour_de_jeu == "blanc":
        canvas_couleur.configure(bg="white")
    elif main_client.tour_de_jeu == "noir":
...
def pop_up_choix_piece(case : int):
    ...
    radio_choix_piece_pion = Radiobutton(frame_choix_piece, image=(image_pion_blanc if main_client.couleur_joueur2 == "blanc" else image_pion_noir), variable=piece_voulu, value=(constant.pion_blanc if main_client.couleur_joueur2 == "blanc" else constant.pion_noir))
    radio_choix_piece_pion.grid(row=1, column=0, columnspan=2)
    radio_choix_piece_pion.select()

    radio_choix_piece_tour = Radiobutton(frame_choix_piece, image=(image_tour_blanc if main_client.couleur_joueur2 == "blanc" else image_tour_noir), variable=piece_voulu, value=(constant.tour_blanc if main_client.couleur_joueur2 == "blanc" else constant.tour_noir))
    radio_choix_piece_tour.grid(row=1, column=2, columnspan=2)

    radio_choix_piece_cavalier = Radiobutton(frame_choix_piece, image=(image_cavalier_blanc if main_client.couleur_joueur2 == "blanc" else image_cavalier_noir), variable=piece_voulu, value=(constant.cavalier_blanc if main_client.couleur_joueur2 == "blanc" else constant.cavalier_noir))
    radio_choix_piece_cavalier.grid(row=1, column=4, columnspan=2)

    radio_choix_piece_fou = Radiobutton(frame_choix_piece, image=(image_fou_blanc if main_client.couleur_joueur2 == "blanc" else image_fou_noir), variable=piece_voulu, value=(constant.fou_blanc if main_client.couleur_joueur2 == "blanc" else constant.fou_noir))
    radio_choix_piece_fou.grid(row=1, column=6, columnspan=2)

    radio_choix_piece_reine = Radiobutton(frame_choix_piece, image=(image_reine_blanc if main_client.couleur_joueur2 == "blanc" else image_reine_noir), variable=piece_voulu, value=(constant.reine_blanc if main_client.couleur_joueur2 == "blanc" else constant.reine_noir))
...
def confirme_modif(case, piece_voulu, pop_up):
    ...
    main_client.demande_piece(case, piece_voulu) #on lance la fonction du programme principale qui modifie la piece sur l'échiquier
    pop_up.destroy() #on ferme la pop-up
    main_client.client.envoie(str(main_client.case_selection)+","+str(case)+","+str(main_client.cases[case][0])) #on envoie à l'autre joueur le déplacment effectué ainsi que la pièce qu'il a choisi
    main_client.case_selection = None #on réinitialise la case selectionnée
    dessine_tout()
    main_client.lance_attente() #on lance la fonction d'attente de l'autre joueur
    dessine_tout()


def clique(event):
    if main_client.tour_de_jeu == main_client.couleur_joueur2: #si c'est le tour du joueur 2
        ligne = event.y // constant.CELL_Size #on récupère la ligne où on a cliqué
        colonne = event.x // constant.CELL_Size #on récupère la colonne où on a cliqué
        main_client.selection_case(colonne, ligne) 
        dessine_tout() 
        if main_client.tour_de_jeu == main_client.couleur_joueur1 and main_client.case_piece_demandee == None: #si c'est le tour du joueur 1
            main_client.lance_attente() #on lance la fonction qui permet d'attendre le déplacement de l'autre joueur
            dessine_tout()
...
def connexion():
    ...
    if main_client.client.connected == False: #si le client n'est pas connecté
        bouton_cherche.configure(text="Recherche en cours...")
        fenetre.update()
        main_client.lance() #lance le thread client
        ...
        dessine_tout() #dessine les éléments de l'échiquier
        if main_client.couleur_joueur2 == "noir": # si le joueur est noir, il doit attendre car ce sont les blancs qui commencent toujours
            main_client.lance_attente() #lance la fonction d'attente
            dessine_tout() #dessine les éléments de l'échiquier
        

def quitte():
    """Cette fonction permet de quitter le jeu"""
    if main_client.client.connected == True: #si le client est connecté
        main_client.client.quitter() # on quitte le serveur
...
fenetre = Tk()
fenetre.title("jeu d'échec (client)")
...
canvas_couleur.grid(row=1, column=8, columnspan=2)
...
#cree un bouton
bouton_cherche = Button(frame, text="chercher un adversaire", command=connexion)
